...
export default function SalesInvoices() {
  ...
  const loadData = async () => {
    const [docs, custs, prods] = await Promise.all([
      SaleDocument.filter({ document_type: "factura" }, "-document_date"),
      Customer.list(),
      Product.list()
    ]);
    setDocuments(docs);
  ...
  const getNextDocumentNumber = async (documentDate) => {
    const series = await DocumentSeries.filter({ document_type: "factura_venta", is_active: true });
    if (series.length === 0) {
      alert("No hay series activas para facturas de venta. Por favor, configure una serie primero.");
      return null;
    }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">Gestión de facturas a clientes</p>
          </div>
          <Button 
            onClick={() => {
              setEditingDocument(null);
