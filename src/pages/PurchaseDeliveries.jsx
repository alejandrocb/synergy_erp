...
export default function PurchaseDeliveries() {
  ...
  const loadData = async () => {
    const [docs, supps, prods] = await Promise.all([
      PurchaseDocument.filter({ document_type: "albaran" }, "-document_date"),
      Supplier.list(),
      Product.list()
    ]);
    setDocuments(docs);
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Albaranes de Compra</h1>
            <p className="text-gray-500 mt-1">Gestión de albaranes de recepción</p>
          </div>
          <Button 
            onClick={() => {
              setEditingDocument(null);
