...
export default function SalesQuotes() {
  ...
  const loadData = async () => {
    const [docs, custs, prods] = await Promise.all([
      SaleDocument.filter({ document_type: "presupuesto" }, "-document_date"),
      Customer.list(),
      Product.list()
    ]);
    setDocuments(docs);
  ...
  const getNextDocumentNumber = async (documentDate) => {
    const series = await DocumentSeries.filter({ document_type: "presupuesto_venta", is_active: true });
    if (series.length === 0) {
      alert("No hay series activas para presupuestos de venta. Por favor, configure una serie primero.");
      return null;
    }
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Presupuestos de Venta</h1>
            <p className="text-gray-500 mt-1">Gestión de presupuestos a clientes</p>
          </div>
          <Button 
            onClick={() => {
              setEditingDocument(null);
