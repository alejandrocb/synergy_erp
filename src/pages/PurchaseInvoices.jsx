...
export default function PurchaseInvoices() {
  ...
  const loadData = async () => {
    const [docs, supps, prods] = await Promise.all([
      PurchaseDocument.filter({ document_type: "factura" }, "-document_date"),
      Supplier.list(),
      Product.list()
    ]);
    setDocuments(docs);
  ...
  const getNextDocumentNumber = async (documentDate) => {
    const series = await DocumentSeries.filter({ document_type: "factura_compra", is_active: true });
    if (series.length === 0) {
      alert("No hay series activas para facturas de compra. Por favor, configure una serie primero.");
      return null;
    }
